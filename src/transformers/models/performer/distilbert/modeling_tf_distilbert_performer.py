...
class TFEmbeddings(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        self.word_embeddings = TFSharedEmbeddings(
            config.vocab_size, config.hidden_size, initializer_range=config.initializer_range, name="word_embeddings"
        )  # padding_idx=0)
...
class TFMultiHeadSelfAttention(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        self.q_lin = tf.keras.layers.Dense(
            config.hidden_size, kernel_initializer=get_initializer(config.initializer_range), name="q_lin"
        )
        self.k_lin = tf.keras.layers.Dense(
            config.hidden_size, kernel_initializer=get_initializer(config.initializer_range), name="k_lin"
        )
        self.v_lin = tf.keras.layers.Dense(
            config.hidden_size, kernel_initializer=get_initializer(config.initializer_range), name="v_lin"
        )
        self.out_lin = tf.keras.layers.Dense(
            config.hidden_size, kernel_initializer=get_initializer(config.initializer_range), name="out_lin"
        )
    ...
    def call(self, query, key, value, mask, head_mask, output_attentions, training=False):
        ...
        # Performer does not support outputting attention
        return (context,)
...
class TFFFN(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        self.lin2 = tf.keras.layers.Dense(
            config.hidden_size, kernel_initializer=get_initializer(config.initializer_range), name="lin2"
        )
...
class TFTransformerBlock(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        ), f"Hidden size {config.hidden_size} not dividable by number of heads {config.n_heads}"
...
class TFDistilBertPerformerForMaskedLM(TFDistilBertPerformerPreTrainedModel, TFMaskedLanguageModelingLoss):
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.vocab_transform = tf.keras.layers.Dense(
            config.hidden_size, kernel_initializer=get_initializer(config.initializer_range), name="vocab_transform"
        )
